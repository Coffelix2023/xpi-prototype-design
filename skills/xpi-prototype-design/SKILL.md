...
### hifi 阶段
> **先过 §7 的判据再进本表。** 判定为跳过 hifi 时，本表一个技能都不要调，wireframe 直接进成品。
...
判据不是「保真度够不够」，而是：**这一轮还剩什么未定决策，以及谁来做这个决策。** 保真度阶梯不是默认路径。

原型 HTML 不会被成品继承（自包含 HTML 与 React/Tailwind 之间转换有损），所以每一层跃迁都是**纯重写，只继承决策**。多走一层却不产出新决策，就是白亏一次重写。

| 触发条件 | 本轮的保真度决定 | 产出范围 |
| :--- | :--- | :--- |
| 决策者是你自己，且视觉系统已定（`THEMES.md` 或组件库默认） | **跳过 hifi** | wireframe 即最后一个原型阶段 |
| 有外部决策者（客户/老板/设计），且他们不读代码 | **限缩 hifi** | 1 屏完整；不要求覆盖全部页面与全部状态 |
| 视觉方向有 ≥2 个真候选 | **1 屏 × N 个方向** | 每方向一屏用于比选；不为任一候选建完整流程 |

选定后把**选了哪一支、为什么**写进 `plan.md`，让后读的人能区分「刻意跳过」和「忘了做」。

> hifi 是**设计系统缺位时的替代品**。视觉已经由组件库默认值 + token 契约固定时，它想干的活已经被那一层干掉了，剩下的只有重写成本。

### 跳级的义务前移

跳过 hifi **不等于少做一层**，而是把 hifi 的决策义务前移到 wireframe。漏掉这一半，跳级就真的变成偷工：

- [ ] 状态覆盖：正常 / 空 / 加载 / 错误，各自独立视图
- [ ] 文案贴近真实长度（中文 12–20 字，禁 lorem ipsum）
- [ ] 可修改元素的语义 ID 已分配（HTML `id` 属性 = 字典短码或全路径）

上列任一项缺失时，**不得**把跳过 hifi 说成合理取舍，也**不得**报「本轮完成」。
...
| 改的是 | 性质 | 处理 |
| :--- | :--- | :--- |
| 块顺序、信息层级、内容位置 | **流程缺陷**：结构本该继承 | 登记 `DELTA.md` 并如实报告，不要当成正常产出 |
| 字体、间距、密度、颜色 | 正常：线框里本来就没有这些信息 | 不登记结构偏离 |

若某处外观调整在成品实现里**还要再做一遍**，把它记进 `plan.md` 标为「延后到成品」，不要排进原型阶段。同一件事做两次是纯亏。
...
进度就是 `impl` 本身，**没有第二份清单**：缺失即未推进、存在即已推进。不另建进度文件，也不把搬运任务塞进 `tasks.md`——那个文件每轮深挖后被覆写，而其中的任务行受计划闸门管；搬运写的是成品源码，闸门不管。
...
- **推进对包含边完整**：推进一个元素时，它的 `children` 里每个 `status: locked` 的子元素必须**同轮一并推进**。「卡片 `⋯` 菜单只搬了触发器、6 个菜单项没搬」就是这条被违反的典型形态。不想推进某个子项，就**别把它置为 `locked`**——「本就不打算进生产」的合法表达是状态，不是新增字段。
...
**搬运核对**（元素推进到成品之后跑一次）——字典说搬了，源码里到底有没有：

```text
prototype_promotion_check { project, pageId }
```

按 `impl.path` 去重后扫源码，匹配 `data-semantic-id` 的**全路径**值。匹配坚持**属性形态**——不放宽成「这个值在文件里出现过」：那正是脚本假通过的来源。五类分开报：

| 结果 | 含义 | 怎么办 |
| :--- | :--- | :--- |
| ✗ 已推进但源码里找不到 | 字典说搬了，源码里没有 | 补进源码，或把 `impl` 撤回 |
| ◐ 属性写成动态表达式 | 该文件里有 `data-semantic-id={...}`，而该元素的 id 不是字面量 | 静态核对看不到它。先确认渲染点是否真带该属性：确实带就接受，或用全路径字面量重写 |
| ⊘ 容器搬了、子项没搬 | 父元素有 `impl`，`locked` 子元素没有 | 补搬子项，或把子项改成未 `locked` |
| ＋ 源码有、字典没登记 | 字典落后了 | 登记进字典；**这不等于字典坏了** |
| 命中 | 对上了 | 无事 |

它**只报告**：不写盘、不阻断、不参与写盘许可——`current/` 的写盘许可仍只看本轮的 `gate.json`。字典缺失时它明说「未做核对」，那是**没核对**，不是通过。

◐ 与 ✗ 必须分开看：前者要你**确认**渲染点，后者要你**去写**源码。混在一起时报告长得一样，人没法决定动哪边。

**同一个判定也能在会话外跑**——客户端 CI 用这个入口，不要另写一份脚本（两份判定必然漂移，实测过脚本报 OK 16 个而工具报缺 6 个）：

```bash
# 退出码 fail-closed：0 无差异；1 有未命中或未做核对；2 用法/环境错误
node node_modules/@fx-pi/xpi-prototype-design/bin/check-promotion.mjs --project <slug> [--page <id>]
```

各包的安装路径不同，用本仓实际的包目录替换。脚本 `scripts/check-semantic-ids.sh` 一类自写判定是**降级替代**：只在没法装本包时才用，口径必须与这里一致（只匹配 `data-semantic-id` 的全路径值）。

搬运是跨 session 的长活，核对别攒到最后一次做：**每搬完一个页面跑一次**，未命中项当场处理掉。
**props 契约纪律**（改元素属性时）：
...
- [ ] 搬过元素：`prototype_promotion_check` 已跑，未命中数为 0，或每一个都有已说出口的理由（漏了就补，不补就改字典）
