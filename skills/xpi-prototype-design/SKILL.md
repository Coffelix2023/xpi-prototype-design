...
四个模式，一套版本机制。产物按**项目**组织，每个项目下再分阶段：
...
深挖一结束，先写 `plan.md`（含任务编排节），**再发一张三选一卡**。拿到答复之前，不要往 `current/` 写任何文件，也不要调 `prototype_snapshot`。

| 选项 | 你接下来做什么 |
| :--- | :--- |
| 仅保存 spec 计划和任务编排，稍后执行 | 写完 `plan.md` 就停。回报计划路径与任务编排摘要，并明确说「等你发话再产出」 |
| 保存后立即执行 | 写完 `plan.md`，接着按任务编排产出（§7），收尾照 §8 快照 + 预览 |
| 还有需要补充的 | 不要猜要补什么，直接追问缺哪一块；补完回到本闸门再问一次 |

提问纪律：

- 三个选项**都要在**，不许合并、不许省略——「仅保存」是用户在产出之前唯一的刹车。
- 选「还有需要补充的」后最多再补 2 轮。仍有大块空白就如实说明风险，把决定权留在闸门里。
- 闸门只对**首次产出**（`wireframe` / `hifi`）出现。`update` 走 §9 迭代，不重复过闸门。
- 迟到执行：用户在「仅保存」后说「开始执行 / 继续」，只读 `plan.md` 的任务编排节，从第一个未完成项接着做，**不要重新深挖**。
...
- **迭代**（`update` 模式走这条）：直接改 `current/`，改完 `prototype_snapshot`，在 `plan.md` 里同步更新受影响的章节。
...
- [ ] 首次产出前过了 §5.1 计划闸门，用户明确选了「保存后立即执行」
