...
6. Once an element has been promoted, run `prototype_promotion_check { project, pageId }` — or its CI twin, `node node_modules/@fx-pi/xpi-prototype-design/bin/check-promotion.mjs --project <slug>`, which calls **the same** judging function and the same renderer. It resolves each registered `impl.path` and matches the **full path** value in `data-semantic-id`, in **attribute form only** — never "the value appears somewhere in the file", which is exactly how a constant table fakes a pass. Five classes are reported separately: promoted but absent from source, attribute written as a dynamic expression (invisible to a static check), a container promoted while its `locked` children were not, source ids missing from the dictionary, and matches. Read-only — it never writes, never blocks, and never takes part in the write gate. The CLI exits non-zero for `missing` / `unreadable` as well, so CI cannot go green on a check that never ran.

> **Do not write your own judging script.** A second implementation of the same rule drifts — observed in practice: a hand-rolled `rg`-based script reported `OK: 16` while the tool reported `10 matched, 6 missing` on the same source. Two classes need different fixes and must not be merged: `missing_in_source` means go write the source; `dynamic_attribute` means go confirm what the render point actually emits. If you cannot install this package, copy the criterion exactly — `data-semantic-id` with the **full path** value, attribute form only.
...
| `prototype_promotion_check` | The registered production landing spots (`impl.path`) plus those source files | Nothing (read-only) | Never writes, never blocks, and never participates in the write gate. Reports five classes separately: promoted but not found in source, attribute written as a dynamic expression, container promoted while its `locked` children were not, source `data-semantic-id` values missing from the dictionary, and matches. Matches the **full path** value, attribute form only — a short code in source, or the value merely appearing in a constant table, is reported as unregistered rather than as a hit. A missing or unreadable dictionary says explicitly that no check happened |
| `check-promotion` (CLI) | Same inputs as above, from `--project` / `--page` / `--cwd` on the command line | Same output as the tool: **one** judging function and **one** renderer | Never writes. Exit code is fail-closed: `clean` → 0; `issues` → 1; `missing` / `unreadable` → 1; usage or environment error → 2. Package path varies by install — `node node_modules/@fx-pi/xpi-prototype-design/bin/check-promotion.mjs` |
...
└── src/
    ...
    ├── promotion-check.ts     # prototype_promotion_check: the Pi tool shell (typebox + schema only)
    ├── promotion-core.ts      # the judging + rendering itself: zero typebox, zero Pi runtime
    ├── semantic-map-io.ts     # readProjectMap: zero typebox, so the core can be reused outside a session
