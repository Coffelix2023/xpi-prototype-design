...
每个元素条目 SHALL 包含 `id`（全路径）、`short`（短码）、`label`（中文名）、`type`（类型枚举）、`status`（状态机枚举）、`stage_created`（诞生阶段）、`parent`（父级 id）、`fidelities`（锚点映射）、可选的 `props`（可修改契约）、`behavior`（当前行为）、`i18n_key`、`children`（子元素列表）。
...
系统 SHALL 提供校验工具 `semantic_ui_map_validate(project)`，检查 ID 冲突、alias 重复、引用完整性（parent/children 循环）、状态机合法性、fidelities 路径存在性。
...
系统 SHALL 提供解析器 `semantic_ui_map_parse(input, project, context?)`，接受短码、全路径或中文别名，返回 `{ matched: true, element }` 或 `{ matched: false, candidates: [...] }` 或 `{ matched: false, status: 'unregistered' }`。

#### Scenario: 短码直接解析

- **WHEN** 输入为 `P1-2-B1`
- **THEN** 解析器 MUST 返回该短码对应的完整元素信息
...
- **WHEN** 输入为 "按钮" 且提供 `context: { page: 'chat' }`
