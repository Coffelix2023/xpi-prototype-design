...
本文件只描述 `xpi-prototype-design` 的仓库级 GitHub 约束。
...
- `xpi-prototype-design` 目前按**阶段一**处理：单人快速迭代优先。
- 当前仓库不启用 `release-please`。
- **本仓库默认在 `main` 上直接提交与推送，不建分支。** 提交与推送前一律走 `docs/GIT-WORKFLOW.md` §3 的 Git 卫生检查点。
- 需要分支提交、PR 评审或发布流程时，**由用户显式提出**；Agent 不得自行引入分支或 PR。
- 如果以后切到阶段二，再把流程改成「分支 + PR + 人工合并」为主。

## 本仓库约束

- 默认在 `main` 上直接提交与推送；不建分支（阶段一）。
- ruleset 的 `Block force pushes` / `Restrict deletions` 始终生效：放行普通提交，禁止历史重写与删除分支。这层防线不需要也不应该被绕过。
